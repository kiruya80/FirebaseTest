...
import com.ulling.firebasetest.common.ApiUrl;
...
public class RetrofitService extends QcBaseRetrofitService {

    private static YoutubeApi apiInterface;
    public static Retrofit retrofit = null;

    public synchronized static YoutubeApi getInstance() {
    ...
    private static YoutubeApi provideApiService() {
        return getRetrofit(ApiUrl.BASE_YOUTUBE_URL, provideClient(false)).create(YoutubeApi.class);
    }
...
//                        .header("Content-Type", "application/x-www-form-urlencoded")
                        .method(original.method(), original.body())
                        .url(url).build();
                QcLog.e("request url " + request.url());
                return chain.proceed(request);
            }
        })
